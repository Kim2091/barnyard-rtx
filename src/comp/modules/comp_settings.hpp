...
namespace comp
{
	...
	public:
		...
		struct var_definitions
		{
			...
			variable object_nucull_distance = {
				"object_nucull_distance",
				("Distance (radius around player) where culling of objects is disabled. Do not overdo without increased model rendering limits.\n"
				 "Stuff infront might start to disappear otherwise!"),
				15.0f
			};

			variable world_nucull_distance = {
				"world_nucull_distance",
				("Distance (radius around player) where culling of world meshes is disabled"),
				50.0f
			};
