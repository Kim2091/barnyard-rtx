...
namespace comp
{
	...
	public:
		...
		bool save_fvf(IDirect3DDevice9* device)
		{
			if (fvf_set_) {
				return false;
			}
		...
		// save D3DTS_WORLD
		void save_world_transform(IDirect3DDevice9* device)
		{
		...
		// restore FVF
		void restore_fvf(IDirect3DDevice9* device)
		{
			if (fvf_set_) 
			{
				device->SetFVF(fvf_);
